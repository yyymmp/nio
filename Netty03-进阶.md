...
缺点，处理字符数据比较合适，但如果内容本身包含了分隔符（字节数据常常会有此情况），那么就会解析错误   因为也需要遍历找到分隔符
...
第一个参数: 消息长度偏移量,与第一个字节的偏移,如果从一开始就表示消息长度,那么这个值就是0

第二个参数:表示消息内容长度占几个字节

第三个参数:消息字节长度到内容间隔几个字节

第四个字节:解码后是否剥离,就是将非消息内容剥离出去,从开头到消息内容开始的地方占多少字节
