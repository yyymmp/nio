...
分散读取，有一个文本文件 3parts.txt  就是读取或者写入时 使用buffer数组
...
#### ⚠️ FileChannel 工作模式 ,与selector相关的配合非阻塞的
...
* 但非阻塞模式下，即使没有连接建立，和可读数据，线程仍然在不断运行，白白浪费了 cpu,因为上方的while循环一直在执行,但是并没用客户端连接
...
1 就是数据消息的边界,将消息进行拆分,得到完成消息

2 bytebuf大小如何分配比较合理(netty解决的问题)
...
**根据此标准来判断阻塞是同步还是异步,得到**以下结果:同步阻塞、同步非阻塞、同步多路复用、异步阻塞（没有此情况）、异步非阻塞
