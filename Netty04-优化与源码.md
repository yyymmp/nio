...
* 属于 ServerSocketChannal 参数,用在服务器端
...
**注意,只有accept事件处理不了时,才会堆积到全连接队列**

netty 中

可以通过  option(ChannelOption.SO_BACKLOG, 值) 来设置大小,是在程序中控制客户端的个数
...
* 属于操作系统参数  限制一个进程能打开所有文件的个数  临时调整



#### 4）TCP_NODELAY

* 属于 SocketChannal 参数,底层为了发送效率更高,会将多个小包合并在一起发送,建以设置为false



#### 5）SO_SNDBUF & SO_RCVBUF:发送和接受缓冲区,决定了滑动窗口的大小
